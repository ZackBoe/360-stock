exports.task = {
    ...
    run: function(api, params, next){
        ...
        request({url:'https://www.kimonolabs.com/api/6uag53ws', qs:{apikey:api.config.keys.kimono}}, function (error, response, body) {
        ...
        request({url:'https://www.kimonolabs.com/api/byb181li', qs:{apikey:api.config.keys.kimono}}, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                data = JSON.parse(body);
                if(data.results[360][0].price['class'].indexOf('disabled') > -1){
